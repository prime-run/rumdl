        ...
        for (line_idx, line_info) in ctx.lines.iter().enumerate() {
            ...
            if let Some(blockquote) = &line_info.blockquote {
                ...
                if blockquote.has_multiple_spaces_after_marker {
                    ...
                    let extra_spaces_start =
                        blockquote.marker_column + blockquote.nesting_level + 1; // Position after all '>' markers + 1 for the first space
                ...
                let malformed_attempts =
                    self.detect_malformed_blockquote_attempts(&line_info.content);
...
impl MD027MultipleSpacesBlockquote {
    ...
    fn detect_malformed_blockquote_attempts(
        &self,
        line: &str,
        ...
        for (pattern, issue_type) in MALFORMED_BLOCKQUOTE_PATTERNS.iter() {
            if let Some(cap) = pattern.captures(line) {
                ...
                if let Some((fixed_line, description)) =
                    self.extract_blockquote_fix_from_match(&cap, issue_type, line)
                {
    ...
    fn looks_like_blockquote_attempt(&self, original: &str, fixed: &str) -> bool {
        ...
        let content_after_markers = trimmed_original
            .trim_start_matches('>')
            .trim_start_matches(' ');
        ...
        let word_count = content_after_markers
            .split_whitespace()
            .count();
        if word_count < 3 {
